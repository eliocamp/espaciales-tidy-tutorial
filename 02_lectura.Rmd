...
Si de casualidad los datos que vas a usar están en un formato de texto o excel, podés usar cualquier función para leerlos que te resulte cómoda. R base tiene `read.csv()`, si vivís en el mundo de Tidyverse podés usar `read_csv()`.

Leamos la temperatura máxima para distintas estaciones meteorológicas del país:

```{r}
observaciones <- read.csv("datos/tmax_20220520.csv") #  Ó read_csv()
```
...
En los casos donde esto no ocurre podés aprovechar las opciones de las funciones para definir por ejemplo el tipo de dato de cada columna al momento de leer los datos. Veamos que pinta tienen, luego podemos revisar en detalle como generar el gráfico. 

```{r}
mapa <- mapa <- rnaturalearth::ne_countries(country = c("argentina", "falkland islands"), 
                                    returnclass = "sf")
...
Muchos datos vienen en este formato, sobre todo cuando se trata de datos puntuales. Sin embargo, tienen una desventaja, no tienen metadatos asociados o al menos *no vienen con el archivo*.
